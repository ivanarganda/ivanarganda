<!-- 🌍 GITHUB PROFESSIONAL PROFILE | Iván González Vallés -->

![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=23&duration=2800&pause=1000&color=00B4FF&center=true&vCenter=true&width=650&lines=👋+Hi%2C+I'm+Iván+González+Vallés;💡+Data+Analyst+%7C+Python+%26+Web+Developer;🚀+AI+and+Big+Data+Enthusiast;🎯+Turning+data+into+intelligent+experiences!)

## ⚡ Skill Snapshot

🎨 **Tech Stack Overview**

`PHP` • `Python` • `Flask` • `FastAPI` • `Flet` • `Pandas` • `Power BI` • `Plotly` • `SQLite` • `HTML` • `CSS` • `JavaScript` • `React`

## 🎯 My Progress

| Skill | Progress | Description |
|-------|-----------|--------------|
| 🐘 **PHP Development** | <img src="progress-94.svg" width="100"> | Building web apps, APIs, and scripting automation tools |
| 🐍 **Python Development** | <img src="progress-85.svg" width="100"> | Building apps, APIs, and automation tools |
| ⚙️ **Flask / FastAPI** | <img src="progress-70.svg" width="100"> | Scalable backends, authentication, async requests |
| 📊 **Excel / Data Analytics** | <img src="progress-81.svg" width="100">| KPI dashboards, Pivot tables, formules |
| 📊 **Power BI / Data Analytics** | <img src="progress-81.svg" width="100">| KPI dashboards, DAX, time intelligence |
| 🤖 **Machine Learning** | <img src="progress-50.svg" width="100"> | Regression, forecasting, model deployment |
| 🎨 **UI/UX & Web Design** | <img src="progress-94.svg" width="100"> | Glassmorphism, modern layouts, responsive design |
| 🚀 **Async & API Integration** | <img src="progress-70.svg" width="100"> | AsyncIO, threading, API pipelines |


[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:ivangonzalezvalles@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/ivanarganda)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ivan-gonzalez-valles)


## 🧠 About Me / Sobre mí

> “Transforming data into insight, and ideas into innovation.”

**English:**  
I’m a **Data Analyst and Python & Web Developer** passionate about building intelligent, data-driven, and visually appealing applications.  
My work blends **analytics, AI, and UX design** to create meaningful digital experiences that help businesses make smarter decisions.

**Español:**  
Soy **analista de datos y desarrollador Python & Web**, especializado en construir soluciones modernas que combinan datos, inteligencia artificial y diseño.  
Desarrollo proyectos que integran analítica avanzada, interfaces intuitivas y herramientas inteligentes.

---

## ⚙️ Tech Stack

**Languages & Tools:**  
Python · Flask · FastAPI · SQLite · HTML · CSS · JavaScript · React · Git · GitHub · Pandas · Plotly · scikit-learn · Power BI

---

## 🧩 Featured Projects / Proyectos Destacados

| 🚀 Project | 💡 Description | 🛠️ Stack |
|-------------|----------------|-----------|
| [OmniMind](https://github.com/ivanarganda/proyectosFlet/tree/main/OmniMind) | Modular app combining analytics, AI, web interfaces and games | Python · Flet · Flask · SQLite · JWT |
| [DataMind](https://github.com/ivanarganda/DataMind) | Smart suite for data cleaning and dynamic dashboard generation | Python · Pandas · Plotly · Power BI |
| [Web Dashboard Kit](https://github.com/ivanarganda/WebDashboardKit) | Responsive dashboards for analytics visualization | HTML · CSS · JS · Flask |
| [Crypto Predictor](https://github.com/ivanarganda/CryptoPredictor) | Time-series prediction using Prophet and Pandas | Python · Prophet · Matplotlib |

---

## 🌍 What I Do / Lo que hago

| 💼 Area | ⚙️ Tools | 🎯 Goal |
|----------|-----------|-------------|
| **Data Analytics** | Pandas · Power BI · Excel · DAX | Transform data into insights |
| **Web Development** | Flask · FastAPI · Flet · HTML/CSS/JS | Create modern, responsive apps |
| **Machine Learning** | scikit-learn · Prophet | Build predictive and intelligent models |
| **UI/UX Design** | Glassmorphism · Gradients · Minimalism | Craft elegant, intuitive interfaces |

---

## 📈 Learning Journey / Trayectoria de Aprendizaje

**Currently mastering**
- Advanced **Power BI (DAX, modeling, security)**
- **Async Python** (asyncio, threading, requests_async)
- **FastAPI + Flask** backend architectures  
- **Front-End Integration** (HTML, CSS, JS)
- **Machine Learning Pipelines** (Regression, PCA, NLP)

**Next goals**
- Deploy **OmniMind Web + Desktop Hybrid**
- Publish first **PyPI data toolkit**
- Build my **personal portfolio website**
- Reach **C1 English level** (Aptis / Cambridge)

---

## 🚀 My Projects Portfolio

## Web development ( HTML - CSS - Javacript - PHP - MySQL - React - JQuery
| 🔗 Project | 💡 Description | 🛠️ Stack
|-------------|----------------|-----------|
| [Flask Portfolio](https://github.com/ivanarganda/portfolio/tree/main) | Personal web with contact form and project viewer | React · HTML · CSS · JS |
| [Flask Portfolio](https://github.com/ivanarganda/controlfichajes) | Clock in-out app | PHP · HTML · CSS · JS |

## Excel Management and Data Anañyst ( VBA - KPI Dashboard - Formules - Pivot tables )
| 🔗 Project | 💡 Description | 🛠️ Stack
|-------------|----------------|-----------|
| [Concesionary Dashboard Excel](https://github.com/ivanarganda/proyectosExcel/tree/main/concesionario) | Vehicles in sale and his customers | KPI · Percentages · Progress · Charts |
| [Concesionary Dashboard Excel](https://github.com/ivanarganda/proyectosExcel/tree/main/tiendas) | Stores in sale and his customers | KPI · Percentages · Progress · Charts |
| [Concesionary Dashboard Excel](https://github.com/ivanarganda/proyectosExcel/tree/main/transportes) | Transport about logistic | KPI · Percentages · Progress · Charts |

## Power BI Data Anañyst ( DAX - Modelling - M - KPIS - Visual Objects
| 🔗 Project | 💡 Description | 🛠️ Stack
|-------------|----------------|-----------|
| [Sales Dashboard Power BI](https://github.com/ivanarganda/SalesDashboard) | Interactive BI dashboard | Power BI · DAX |

## Python Data Analyst and development ( Pandas - Machine learning - Cohere CHAT - Flask - Flet )
| 🔗 Project | 💡 Description | 🛠️ Stack
|-------------|----------------|-----------|
| [OmniMind](https://github.com/ivanarganda/proyectosFlet/tree/main/OmniMind) | Multi-module data & AI platform | Python · Flet · Flask · SQLite |
| [DataMind](https://github.com/ivanarganda/DataMind) | Automated data cleaning & dashboard builder | Python · Pandas · Plotly |
| [Crypto Predictor](https://github.com/ivanarganda/CryptoPredictor) | Prophet-based forecasting tool | Python · Prophet · Pandas |

---

## 📄 My Professional CV

[![View CV](https://img.shields.io/badge/Open%20CV-PDF-blue?style=for-the-badge&logo=adobeacrobatreader&logoColor=white)](https://github.com/ivanarganda/portfolio/blob/main/CV_IvanGonzalez.pdf?raw=true)

*(Click to open in a new tab)*

---

## 🏆 Highlights / Logros

- 🎓 Selected among top students in **Big Data & AI Program (Inserta Empleo / ONCE)**
- 🧠 Creator of **OmniMind**, an integrated data, AI & game ecosystem
- 📊 Built professional **Power BI dashboards** with advanced KPIs and ranking logic
- 💡 Recognized for **UX-driven development and data storytelling**
- ⚡ Passionate sprinter — fast in running and in execution 😉

---

## 💬 Soft Skills

Analytical Thinking · Problem Solving · Creativity  
Team Collaboration · Empathy · Proactivity  
Communication (Spanish 🇪🇸 / English 🇬🇧 B2–C1)  
Curiosity · Continuous Learning · Adaptability  

---

## 💫 Personal Manifesto

> “He tenido miedo muchas veces.  
> Miedo a fallar, a no ser suficiente, a que me juzguen o me cierren puertas.  
>  
> Pero aún con miedo, seguí caminando.  
> A veces lento, a veces cansado, pero siempre con el corazón puesto en avanzar.  
>  
> No busco ser perfecto: busco ser constante.  
> No busco aprobación: busco propósito.  
>  
> 🌱 No me rindo. ✨ Sigo adelante. 💡 Y seguiré creando, aprendiendo y demostrando que mi voz y mi esfuerzo valen.”

---

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=ivanarganda&show_icons=true&theme=tokyonight)  
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=ivanarganda&layout=compact&theme=tokyonight)

---

## 🌐 Connect With Me / Conecta Conmigo

[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:ivangonzalezvalles@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/ivanarganda)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0078D4?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ivan-gonzalez-valles)

---

### 🌟 “Data · Design · Development — building intelligent experiences.”
